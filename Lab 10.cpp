...
class bst{
	struct treeNode{
		...
		int priority;
		treeNode(int value, int level) : data(value), left(NULL), right(NULL), priority(level) {}
...
/*	void insert(const T & data){
	...
	void Treap::insert(treeNode* &p, int value)
	{
    	if (p == NULL) {
       	 ...
       	 if (value == p->data) {
       	     ...
       	     insert(p->lchild, value);
       	 } else {
    	        insert(p->rchild, value);
	        }
	
       	 if(p->lchild && p->lchild->priority > p->priority) {
       	     rightRotate(p);
       	 } else if(p->rchild && p->rchild->priority < p->priority) {
       	     leftRotate(p);
       	 }
    	}
	}
